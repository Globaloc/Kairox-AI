...
export default defineEventHandler(async (event) => {
	...
	const objectPrompt = String(formData.get("objectPrompt") || "").trim();
	const isPro = await validateUserStatus(event.context.user.id);

	if (!objectPrompt) {
		throw createError({
			statusCode: 400,
			statusMessage: "Object description is required",
		});
	}
	...
	try {
		...
		const encodedPrompt = encodeURIComponent(objectPrompt);
		const objectRemovedUrl = cloudinary.url(result.public_id, {
			secure: true,
			resource_type: "image",
			transformation: [
				{
					effect: `gen_remove:prompt_${encodedPrompt};multiple_true;remove-shadow_true`,
				},
			],
		});
		...
		return {
			...
			secureUrl: objectRemovedUrl,
		};
	} catch (error: any) {
		const message = error?.message || "Cloudinary upload failed";

		console.error("Cloudinary object removal upload failed", {
			message,
